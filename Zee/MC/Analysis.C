...
void CLoop::Fill(double weight, int z_sample) {
    ...
    if (n_electrons==2 && q_mu0!=q_mu1 && elec_id  && n_ljets>=2 && n_ljets<=3 && NOMINAL_pileup_combined_weight > -10){
      //angles
      ...
      if (mjj>=250 && trigger_decision && trigger_match) {
        ...
        if (inv_mass < 101 && inv_mass > 81){cuts[12]=1;} // Z-peak mass range 81 < m < 101 GeV. // Mid Range inv_mass < 160 && inv_mass >= 101 
        if (event_number%2==0){

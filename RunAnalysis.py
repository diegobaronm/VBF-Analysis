...
def createParser():
    ...
    parser.add_argument("--log-file", help="Path to a file where log messages are also written.", type=str, default=None)
...
def main():
    ...
    from AnalysisCommons.Logger import Logger
    if parser.log_file is not None:
        Logger.enableFileLogging(parser.log_file)
        INFO.log("File logging enabled: %s" % parser.log_file)

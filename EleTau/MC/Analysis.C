...
void CLoop::Book() {
  ...
  normPtDifferenceContainer =  histogramContainer("normPtDifference"," (tau-e)/(tau+e) pT ",200,-1.0,1.0,notFull);
  metToDilepnuRatioContainer =  histogramContainer("metToDilepnuRatio","MET/(tau+e+neutrinos) pT",50,0,5.0,notFull);
  metToDilepRatioContainer =  histogramContainer("metToDilepRatio","MET/(tau+e) pT",50,0,5.0,notFull);
  ...
  elecPdgIDContainer =  histogramContainer("elecPdgID","Electron truth pdgID",50,-25,25,notFull);
  ...
  leptau_massContainer =  histogramContainer("leptau_mass","Invariant mass lepton tau system",200,0,200,m_cutNames,"mlt");
  eBDTContainer =  histogramContainer("eBDT","eBDT Score",100,0,1,m_cutNames,"ebdt");
...
void CLoop::Fill(double weight, int z_sample, const std::string& sampleName) {
  ...
  if (correctCharge && n_electrons==1 && n_taus_rnn_loose>=1 && lepton_id && n_ljets>=2 && n_ljets<=3 && mjj>=250 && trigger_decision && trigger_match){
    ...
    if (sampleName.find("truth")!=std::string::npos){
      ...
      trueMass = Kinematics::Mass({&truth_lep_p4, tau_0_truth_total_p4});
    ...
    if (tauTauTopology == Kinematics::TauTauTopology::INSIDE){
      lepnu_ptContainer.Fill(elec_0_p4->Pt()+nu_lep_p4.Pt(),weight,cutsVector);
      taunu_ptContainer.Fill(tau_0_p4->Pt()+nu_tau_p4.Pt(),weight,cutsVector);
      sum_ptContainer.Fill(elec_0_p4->Pt()+nu_lep_p4.Pt()+tau_0_p4->Pt()+nu_tau_p4.Pt(),weight,cutsVector);
      Z_pt_reco_iNotFullContainer.Fill(Z_pt,weight,notFullCutsVector);
    } else {
      lepnu_ptContainer.Fill(elec_0_p4->Pt()+nu_lep_p4.Pt(),weight,cutsVector);
      taunu_ptContainer.Fill(tau_0_p4->Pt()+nu_tau_p4.Pt(),weight,cutsVector);
      sum_ptContainer.Fill(elec_0_p4->Pt()+tau_0_p4->Pt()+nu_lep_p4.Pt()+nu_tau_p4.Pt(),weight,cutsVector);
  ...
  if (ql!=qtau && n_electrons==1 && n_taus_rnn_loose>=1 && lepton_id && n_ljets>=2 && n_ljets<=3 ){
    ...
    if (mjj>=250 && trigger_decision && trigger_match) {
      ...
      if (signal_events){
        ...
        //PT BALANCE
        double pt_bal{0};
        double scalarSum = tau_0_p4->Pt()+elec_0_p4->Pt()+ljet_0_p4->Pt()+ljet_1_p4->Pt()+bjet_0_p4->Pt();
        TLorentzVector vectorSum = (*tau_0_p4)+(*elec_0_p4)+(*ljet_0_p4)+(*ljet_1_p4)+(*bjet_0_p4);
